  ...
  class ContentProfileTestDataLoader
    ...
    def create_full_content_profile
      ...
      primary_topics = [
        ...
        {name: "Branch", description: "Account Actions for a specific branch", value_based_y_n: "Y",
            topic_type_opts_attributes: {
            "0" => {value: "1601", type_name: "Branch", description: "Branch Account"},
            "1" => {value: "1602", type_name: "Branch", description: "Branch Account"} }
        },
        {name: "Branch",  description: "Branch Operations for a specific branch",  value_based_y_n: "Y",
            topic_type_opts_attributes: {
                "0" => {value: "0037", type_name: "Branch", description: "North Branch Number"},
                "1" => {value: "0040", type_name: "Branch", description: "West Branch Number"} }
        }
      ]
      ...
      secondary_contents = [
        {name: "Commission", description: "Monthly Commission Reports and Files", value_data_type: "Integer",
            content_type_opts_attributes: {
            "0" => {value: "*.pdf", type_name: "Commission", description: "Document store Branch Experience Document Type ID"} }
          },
        {name: "Notification", description: "Email Notification of Related Events", value_data_type: "String",
            content_type_opts_attributes: {
            "0" => {value: "AdvCancel", type_name: "Notification", description: "Advance Cancel" } }
      ...
      secondary_topics = [
          ...
          {name: "Branch", description: "Account Actions for a specific branch", value_based_y_n: "Y",
                 topic_type_opts_attributes: {
                     "0" => {value: "1602", type_name: "Branch", description: "All Producer Accounts"} }
          }
      ]
      ...
      cpep_recs.each_with_index {|rec,idx| rec.topic_type_description = t_recs[idx].description;   rec.topic_value = t_recs[idx].topic_type_opts.map(&:value).flatten }
      cpep_recs.each_with_index {|rec,idx| rec.content_type_description = t_recs[idx].description; rec.content_value = c_recs[idx].content_type_opts.map(&:value).flatten }
      ...
      cpes_recs.each_with_index {|rec,idx| rec.topic_type_description = t_recs[idx].description;   rec.topic_value = t_recs[idx].topic_type_opts.map(&:value).flatten }
      cpes_recs.each_with_index {|rec,idx| rec.content_type_description = t_recs[idx].description; rec.content_value = c_recs[idx].content_type_opts.map(&:value).flatten }

      c_recs = ContentType.create!(vendor_contents)
      t_recs = TopicType.create!(vendor_topics)
      cpev_recs = ContentProfileEntry.create!(cpev)
      cpev_recs.each_with_index {|rec,idx| rec.topic_type_description = t_recs[idx].description;   rec.topic_value = t_recs[idx].topic_type_opts.map(&:value).flatten }
      cpev_recs.each_with_index {|rec,idx| rec.content_type_description = t_recs[idx].description; rec.content_value = c_recs[idx].content_type_opts.map(&:value).flatten }
