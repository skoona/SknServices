...
namespace :access_registry do
  ...
  namespace :preload do
    ...
    task :drop_profiles => :environment do
      Utility::ContentProfileTestDataLoader.new().clear_existing_data
    end

    desc 'Creates the ContentType and ContentTypeOpts entries..'
    task :create_content_types => :environment do |t, args|
      Utility::ContentProfileTestDataLoader.new().create_content_types_and_options
    end

    desc 'Creates the TopicType and TopicTypeOpts entries..'
    task :create_topic_types => :environment do |t, args|
      Utility::ContentProfileTestDataLoader.new().create_topic_types_and_options
    end

    desc 'Creates ProfileType records.'
    task :create_profile_types =>  :environment do |t, args|
      Utility::ContentProfileTestDataLoader.new().create_profile_types
    end

    desc 'Creates ContentProfileEntry records.'
    task :create_content_profile_entries =>  :environment do |t, args|
      Utility::ContentProfileTestDataLoader.new().create_content_profile_entries
    end

    desc 'Creates ContentProfile records.'
    task :create_content_profiles =>  [:environment,
                                       :drop_profiles,
                                       :create_profile_types] do |t, args|
      Utility::ContentProfileTestDataLoader.new().create_full_content_profile
    end
  end
