# spec/services/access_profile_service_spec.rb
#
...
RSpec.describe ApplicationController, "Service routines of AccessProfile.", :type => :controller  do
  ...
  context "Initialization "  do
    ...
    it "#new succeeds with only :controller as init param." do
      expect(AccessProfileService.new({controller: controller})).to be_a(AccessProfileService)
    end
    it "#new succeeds with only :factory as init param." do
      expect(AccessProfileService.new({factory: controller})).to be_a(AccessProfileService)
    end
    it "#new succeeds with all :factory, :controller, and :user as init params." do
      expect(AccessProfileService.new({factory: controller, controller: controller, user: @user})).to be_a(AccessProfileService)
    end

    it "#new fails when :controller is invalid." do
      expect{ AccessProfileService.new({controller: nil}) }.to raise_error(ArgumentError)
    end
    ...
    it "#new recovers when :factory is invalid, :controller is valid." do
      expect( AccessProfileService.new({factory: nil, controller: controller}) ).to be_a(AccessProfileService)
    end
    it "#factory.access_profile_service returns a proper service object." do
      expect( @factory.access_profile_service ).to be_a AccessProfileService
    end
