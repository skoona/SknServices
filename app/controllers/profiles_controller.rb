...
class ProfilesController < ApplicationController
  ...
  def accessible_content(params)
    puts "HELP accessible_content=> #{params}"
    ...
    if @page_controls.package.success
      render json: results, status: :accepted, layout: false, content_type: 'application/json' and return
    else
      render json: results, status: :conflict, layout: false, content_type: 'application/json' and return
    end
  end
  ...
  def available_content(params)
    puts "HELP available_content=> #{params}"
    @page_controls = access_profile_service.accessible_content(params)
    results = @page_controls.to_hash
    if @page_controls.package.success
      render json: results, status: :accepted, layout: false, content_type: 'application/json' and return
